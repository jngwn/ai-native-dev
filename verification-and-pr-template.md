...
- 검증 매트릭스는 현재 gate, 현재 증거, 현재 한계만 설명하며 changelog로 쓰지 않는다.
- 구현 과정, 과거 상태, review 대화는 PR이나 결정 기록에 남기고 여기서는 필요한 경우 링크만 둔다.
- 같은 검증 상태를 여러 문서에 복사하지 않는다. 검증 매트릭스를 상태의 단일 출처로 두고 기능 문서는 해당 행을 가리킨다.
...
- 검증 상태가 바뀌었다면 과거 설명을 누적하지 않고 현재 gate와 한계로 현행화했는가?
