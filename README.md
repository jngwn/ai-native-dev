...
| 문서 | 역할 |
| --- | --- |
| `README.md` | 저장소 진입점, 문서 역할과 목적별 적용·유지보수 요청 안내 |
...
분석 뒤 반영까지 맡기려면 위 예시의 "아직 수정하지 말고…" 항목을 다음 문장으로 바꾼다.
...
[이 저장소를 유지하는 문서](#이-저장소를-유지하는-문서)의 역할, 문서 간 정합성, 분석·편집 기준을 평가하려면 대상을 명시한다. 참고 프로젝트나 이전 분석 결과가 있으면 판단 근거로 함께 제공할 수 있다.

```text
이 저장소의 유지보수 문서를 repo-review-checklist.md 기준으로 평가해줘.

대상: <위 목록에서 선택한 파일 / 유지보수 문서 전체>
참고 입력: <참고 프로젝트 경로, 이전 분석 결과, 사용 중 겪은 혼동 / 없음>

- 각 대상 문서의 역할과 단일 출처, 문서 간 중복·충돌, 실제 사용에서의 누락을 확인한다.
- 참고 입력은 대상 문서가 소유한 역할의 누락을 확인하는 근거로 사용한다.
- 프로젝트용 계약 템플릿의 변경이 필요하면 별도 후속으로 보고한다.
- 판단 근거, 최소 변경 제안, 반영하지 않을 항목을 보고한다.
- 반영할 가치가 없으면 "반영할 변경 없음"이라고 명시한다.
- 아직 수정하지 마.
```

분석 후 반영까지 맡기려면 마지막 문장을 다음으로 바꾼다.

```text
확인된 구체적인 누락만 지정한 유지보수 문서에 최소 변경으로 직접 반영해줘.
```
